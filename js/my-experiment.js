...
    const results_data = jsPsych.data.get().filter({task_phase: 'test'});

    const trial_values = results_data.values();
...
      <p>ご協力ありがとうございました。以下は、今回の実験で表示された画像と音声の組み合わせです。</p>
...
    for (let i = 0; i < trial_values.length; i++) {

      html += `

        <tr>

          <td style="padding: 8px;">${trial_values[i].image_filename}</td>

          <td style="padding: 8px;">${trial_values[i].sound_pattern}</td>
...
      'cbra3.jpg', 'restau30C.L.jpg', 'room250.jpg', 'int867.jpg',
...
const NUM_AB_PAIRS = 4; // A-Bペアの数 (Aが4試行, Bが4試行)

const NUM_X_TRIALS = 4; // X単独の試行数

const TOTAL_TRIALS = (NUM_AB_PAIRS * 2) + NUM_X_TRIALS; // 総試行回数 = 12
...
const sounds_for_X = shuffled_sounds.slice(NUM_AB_PAIRS * 2, TOTAL_TRIALS);



// --- 画像のサンプリング (各サブカテゴリから均等に抽出) ---

let temp_image_list = [];

const subcategory_refs = [];
...
    subcategory_refs.push({ main: main_cat, sub: sub_cat });
...
// ステップ1: 全10カテゴリから1枚ずつ画像を抽出

subcategory_refs.forEach(ref => {

  const image_array = image_files[ref.main][ref.sub];

  const sampled_image = jsPsych.randomization.sampleWithoutReplacement(image_array, 1);

  temp_image_list.push(sampled_image);

});



// ステップ2: 残りの2枚を、ランダムに選んだ2つの異なるカテゴリから1枚ずつ抽出

const shuffled_refs = jsPsych.randomization.shuffle([...subcategory_refs]);

const extra_refs = shuffled_refs.slice(0, 2); // 2つのカテゴリを追加で選択

extra_refs.forEach(ref => {
...
const selected_images = jsPsych.randomization.shuffle(temp_image_list.flat());
...
  trial_blocks.push({ type: 'AB_PAIR', sound_A: sounds_for_A[i], sound_B: sounds_for_B[i] });
...
  trial_blocks.push({ type: 'X_TRIAL', sound_X: sounds_for_X[i] });

}



// --- ブロックのシャッフルと最終的な刺激リストの生成 ---

let shuffled_blocks = jsPsych.randomization.shuffle(trial_blocks);
...
    stimuli.push({ image: selected_images[image_counter++], sound: block.sound_A, sound_pattern: 'パターンA' });

    stimuli.push({ image: selected_images[image_counter++], sound: block.sound_B, sound_pattern: 'パターンB' });

  } else { // 'X_TRIAL'

    stimuli.push({ image: selected_images[image_counter++], sound: block.sound_X, sound_pattern: 'パターンX' });
...
const all_image_paths = stimuli.map(item => item.image);

const all_sound_paths = stimuli.map(item => item.sound);
...
    <p>画面に画像が表示され、同時に音が再生されます。</p>

    <p>表示された画像が「屋内」の風景か「屋外」の風景かを判断し、対応するボタンをクリックしてください。</p>

    <p>準備ができましたら、何かキーを押して実験を開始してください。</p>
...
        <audio src="${sound_path}" autoplay style="display: none;"></audio>
...
  prompt: '<p>この画像は「屋内」ですか、それとも「屋外」ですか？</p>',
...
  randomize_order: false // stimuli配列は既に意図した順序になっているため、ここではシャッフルしない
